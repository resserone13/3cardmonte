...
# --- to do ---
#fix glitch where you press all in rapidly while the shuffle is finishing and it updates the bank roll to zero and ends the game

#make shuffle more random

#make reveal function

#increase scale when card is flipped and the lower to make it apper to lift off the table
...
class GameOver(Scene):
	def setup(self):
		...
		#self.game_over.alpha=0.0
		#self.new_game.alpha=0.0
...
class Rules(Scene):
	def setup(self):
		...
		self.bg_color = ShapeNode(
			ui.Path.rounded_rect(0, 0, self.size.w * 0.75, self.size.h * 0.75, 10),
			color='#477148', 
			position=(self.size.w/2, self.size.h/2),
			alpha=0.5,
			stroke_color='black',			
			parent=self.rules_node)
		self.bg_color.line_width=4
...
class Main(Scene):
	def setup(self):
		...
		self.status = ''
		self.back_of_card = 'card:BackRed4'
		self.cards = [
			'hide face spade.PNG',
			'hidefacediamonds.PNG', 
			'hidefacehearts.PNG']
		self.status_dict = {
			10_000: 'Hustler', 
			15_000: 'Small Time', 
		...
		self.inc_bet_node = Node(
			parent=self.main_node)
		self.dec_bet_node = Node(
			parent=self.main_node)
		self.multiplier_node = Node(
			parent=self.main_node)
		self.bankroll_node = Node(
			parent=self.main_node)
		self.title_node = Node(
			parent=self.main_node)
		self.pos_node = Node(
			parent=self.main_node)
		self.money_pile_node = Node(
			position= (30, -5),
			parent=self.main_node)
		...
		self.oval_title.shadow =  ('gray', 10, 15, 20)
		
		#yellow boxes surrounding the cards
		yellow_ring_pos = [
			(self.size.w/2 * 0.55, self.size.h/2), 
			(self.size.w/2, self.size.h/2),
			(self.size.w/2 * 1.45, self.size.h/2)]
		
		for yel in yellow_ring_pos:
		...
		#ovals surrounding the positions
		oval_pos = [
			(self.size.w/2 * 0.55, self.size.h / 2 * 0.28),
			(self.size.w/2, self.size.h / 2 * 0.28),
			(self.size.w/2 * 1.45, self.size.h/2 * 0.28)]
			
		for oval in oval_pos:
		...
		#red rectangle for shuffle button
		...
		bet_btn_pos = [
			(self.size.w * .11, self.size.h * 0.55),
			(self.size.w * .11, self.size.h * 0.35)]
			
		for bb in bet_btn_pos:
		...
		self.layered_text('Card MONTE', title_font, title_f_size, self.size.w/2, self.size.h/2 * 1.76, 1, -1, 'black', 'black', 'white')
		self.layered_text('3', three_font, title_f_size, self.size.w/2 *0.70, self.size.h/2 * 1.78, 1, -1, 'black', 'black', 'white')
		...
		self.first_pos = self.layered_text('1st', pos_font, 40, self.size.w/2 * 0.55, self.size.h / 2 * 0.30, 1, -1, 'black', 'black', 'white')
			
		self.second_pos = self.layered_text('2nd', pos_font, 40, self.size.w/2, self.size.h / 2 * 0.30, 1, -1, 'black', 'black', 'white')
		
		self.third_pos = self.layered_text('3rd', pos_font, 40, self.size.w/2 * 1.45, self.size.h/2 * 0.30, 1, -1, 'black', 'black', 'white')
		...
		self.winner_text_2 = LabelNode(
			'WINNER!', 
			font = ('MarkerFelt-Wide', 30), 
			position = (self.size.w/2 + 2, self.size.h/2 -2), 
			color = 'white')
		...
		self.red_x = SpriteNode(
			'emj:Cross_Mark',
			scale= 2)

		self.money_bag = SpriteNode(
			'emj:Money_Bag')
			
		self.bags = [(51,75), (71, 90,), (81, 75)]
		
		for bag in self.bags:
			self.money_bag = SpriteNode(
			'emj:Money_Bag')
			self.money_bag.position=bag
			self.money_pile_node.add_child(self.money_bag)
	...
	def wooden_border(self):
		#wooden block border
		b_border = Node(parent=self)
		b_x_lower = 0
		while b_x_lower <= self.size.w * 1.06:
			wood = SpriteNode(
				'plc:Wood_Block', 
				position = (b_x_lower, 0), 
				z_position= 1, 
				parent=b_border)
			wood_2 = SpriteNode(
				'plc:Wood_Block', 
				position = (b_x_lower, self.size.h * 1.06), 
				z_position= 1, 
				parent=b_border)
			b_x_lower += 50
			
		b_y_left = 0
		while b_y_left <= self.size.h * 1.06:
			wood = SpriteNode(
				'plc:Wood_Block', 
				position = (-6, b_y_left), 
				z_position= 1, 
			...
			wood_2 = SpriteNode(
				'plc:Wood_Block', 
				position = (self.size.w+6, b_y_left), 
				z_position= 1, 
				parent=b_border)
			b_y_left += 50
			
		locations = [
			(self.size.w * 0.015, self.size.h * 0.015), 
			(self.size.w * 0.015, self.size.h * 0.96), 
			(self.size.w * 0.985, self.size.h * 0.98), 
			(self.size.w * 1.003, self.size.h * .000)]

		for location in locations:
			bl_wood = SpriteNode(
				'plc:Wood_Block', 
				parent=b_border)
			bl_wood.position = (location)		
			bl_wood.rotation= 180
	...
	def update(self):
		...
		if self.bankroll_amount == 0 and self.stake_amount == 0:
			self.red_x.run_action(A.fade_to(0, 0.5))
			self.main_node.run_action(A.fade_to(0, 1))
			self.card_pos_1.run_action(A.fade_to(0, 1))
			self.card_pos_2.run_action(A.fade_to(0, 1))		
			self.card_pos_3.run_action(A.fade_to(0, 1))	
			self.second_pos.run_action(A.fade_to(0, 1))	
			go = GameOver()
			go.main = self
	...
	def touch_began(self, touch):
		...
		if touch.location in self.card_pos_1.frame and self.reveal == True:
			...
			if self.cards[0] == 'hide face spade.PNG':
				...
				self.red_x.position=(self.size.w/2 * 0.55, self.size.h/2)
				self.add_child(self.red_x)	
		...
		if touch.location in self.card_pos_2.frame and self.reveal == True:
			...
			if self.cards[1] == 'hide face spade.PNG':
				sound.play_effect('casino:CardShove2')			
				self.add_child(self.winner_text_1)
				self.add_child(self.winner_text_2)
				...
				self.stake_amount_label.text = f'{self.currency_type}{self.stake_amount:,}'
				self.floating_stakes_label.remove_from_parent()
				self.red_x.position=(self.size/2)
				self.add_child(self.red_x)	
				sound.play_effect('game:Error', volume=.18)
				self.wing_money.alpha=0.01
				self.wing_money.scale=0.25
		...
		if touch.location in self.card_pos_3.frame and self.reveal == True:
			...
			if self.cards[2] == 'hide face spade.PNG':
				sound.play_effect('casino:CardShove2')			
				self.add_child(self.winner_text_1)
				self.add_child(self.winner_text_2)
				...
				self.stake_amount_label.text = f'{self.currency_type}{self.stake_amount:,}'
				self.floating_stakes_label.remove_from_parent()
				self.red_x.position=(self.size.w/2 * 1.45, self.size.h/2)
				self.add_child(self.red_x)	
				sound.play_effect('game:Error', volume=.18)
				self.wing_money.alpha=0.01
				self.wing_money.scale=0.25
		...
		y = 61
		self.bags = []
		for i in range(self.bankroll_amount // 10_000):
			if len(self.bags) <= 10:
				self.money_pile_node.add_child(self.money_bag)
				x += random.randint(0, 5)
				y += random.randint(0, 5)
			else:
				self.money_bag.remove_from_parent()
				
		#--- info button ---	
		if touch.location in self.info_btn.frame:
			self.present_modal_scene(Rules())
	...
	def touch_ended(self, touch):
		self.shuffle_rect.alpha = 0.5
		self.inc_bet.font = (self.button_font, 
		30)

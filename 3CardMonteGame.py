...
class GameOver(Scene):
	def setup(self):
		...
		self.new_game = SpriteNode(
			'StartNewGame.PNG', 
			position=(self.size.w/2, self.size.h/2 * 0.32), 
			scale=0.31, 
			parent=self.go_node)			
...
class Main(Scene):
	def setup(self):
		...
		self.first_pos = self.layered_text('1st', pos_font, 40, self.size.w/2 * 0.55, self.size.h / 2 * 0.30, 1, -1, 'black', 'black', 'white', parent=self)
			
		self.second_pos = self.layered_text('2nd', pos_font, 40, self.size.w/2, self.size.h / 2 * 0.30, 1, -1, 'black', 'black', 'white',parent=self)
		
		self.third_pos = self.layered_text('3rd', pos_font, 40, self.size.w/2 * 1.45, self.size.h/2 * 0.30, 1, -1, 'black', 'black', 'white', parent=self)
